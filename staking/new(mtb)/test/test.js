...
describe("Testing Staking contract", function () {
  ...
  it("Simple staking", async function () {
    ...
    const yieldRate = Number(await stakingContract.getMtbUsdRate());
    await stakingContract.connect(signers[10]).setMtbUsdRate(yieldRate * 2);

    result = Number(ethers.utils.formatUnits(
      await stakingContract.getDepositYield(1, true)
    ));
    expect(roundTo(result, 5))
      .to.equal(roundTo(resultStorage.expectedYield / 2, 5));
    await stakingContract.connect(signers[10]).setMtbUsdRate(yieldRate);
    result = Number(ethers.utils.formatUnits(
      await stakingContract.getDepositYield(1, true)
    ));
    expect(roundTo(result, 5))
  ...
  it("Manager settings", async function () {
    ...
    await stakingContract.connect(signers[9])
      .setEtnaUsdRate(123);
    expect(Number(
      await stakingContract.getEtnaUsdRate()
    )).to.equal(123);

    await stakingContract.connect(signers[9])
      .setMtbUsdRate(123);
    expect(Number(
      await stakingContract.getMtbUsdRate()
    )).to.equal(123);
  });
});
