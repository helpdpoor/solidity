...
contract StakingNft is Ownable, IERC721Receiver {
    ...
    mapping (address => mapping (uint256 => uint256)) internal _userTokenRegistry; // userAddress => RegistryIndex => tokenId
    mapping (address => mapping (uint256 => uint256)) internal _userTokenIndexes; // userAddress => tokenId => RegistryIndex
    ...
    uint256 internal _year = 365 * 24 * 3600;
    uint256 internal _apr;
    uint256 internal _shift = 1 ether; // used for exponent shifting when calculation with decimals
    uint256 internal _marketIndex = _shift; // initial market index, used to take care of APR changes
    uint256 internal _marketIndexLastTime = block.timestamp; // last time when market index was updated
    uint256 internal _lockTime; // period when unstake is prohibited
    IERC20 internal _paymentTokenContract;
    ...
    constructor (
        ...
        require(paymentTokenAddress != address(0), 'Token address can not be zero');
        ...
        _paymentTokenContract = IERC20(paymentTokenAddress);
    ...
    function stake (uint256[] memory tokenIds) external returns (bool) {
        ...
        if (depositIndex == 0) {
            ...
            _deposits[depositIndex].unlock = _lockTime + block.timestamp;
        }
    ...
    function withdrawYield (uint256 amount) external returns (bool) {
        ...
        uint256 balance = _paymentTokenContract.balanceOf(address(this));
        require(amount <= balance, 'Not enough contract balance');
        _paymentTokenContract.transfer(msg.sender, amount);
    ...
    function adminWithdrawPaymentToken (uint256 amount) external onlyOwner
    returns (bool) {
        uint256 balance = _paymentTokenContract.balanceOf(address(this));
        require(amount <= balance, 'Not enough contract balance');
        _paymentTokenContract.transfer(msg.sender, amount);
    ...
    function adminSetPaymentTokenContract (
        address contractAddress
    ) external onlyOwner returns (bool) {
        require(contractAddress != address(0), 'Token address can not be zero');
        _paymentTokenContract = IERC20(contractAddress);
        return true;
    }

    function adminSetMarketplaceContract (
        address contractAddress
    ) external onlyOwner returns (bool) {
        require(contractAddress != address(0), 'Token address can not be zero');
        _marketplaceContract = IMarketplace(contractAddress);
        return true;
    }

    function adminSetNftContract (address contractAddress) external onlyOwner returns (bool) {
        require(contractAddress != address(0), 'Token address can not be zero');
        _nftContract = INFT(contractAddress);
        return true;
    }
    ...
    function getEtnaContract () external view returns (address) {
        return address(_paymentTokenContract);
    }
    ...
    function getEtnaBalance () external view returns (uint256) {
        return _paymentTokenContract.balanceOf(address(this));
    }
