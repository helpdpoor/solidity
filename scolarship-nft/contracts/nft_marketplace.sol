...
contract NFTMarketplace is Ownable {
  ...
  modifier onlyOwnerOrAirdropManager() {
    require(msg.sender == _airdrop_manager || msg.sender == owner(),
      "Only airdrop manager or owner can call this function");
    _;
  }
  ...
  */
  ...
  address internal _airdrop_manager;
  ...
  */
  ...
  function getAirdropManager() external view returns(address){
    return _airdrop_manager;
  }

  function setAirdropManager(address newAirdropManager) external onlyOwner returns (bool) {
    require(newAirdropManager != address(0), 'Airdrop manager address should not be zero');
    _airdrop_manager = newAirdropManager;
    return true;
  }
  ...
  */
  ...
  function sellNFTBack (uint256 tokenId) external notContract returns (bool) { //'sell' == burn, burns and returns ETNA to user
    ...
    nftProfiles[index].limit ++;
    if (_nftProfileMinted[profileID] > 0) _nftProfileMinted[profileID] --;
  ...
  */
  ...
  function setOldContractMaxId(uint256 oldContractMaxId) external onlyOwner returns(bool) {
    _oldContractMaxId = oldContractMaxId;
    return true;
  }

  function allowMinting(uint32[] calldata profileIDs, address[] calldata recipients) external
  onlyOwnerOrAirdropManager returns(bool) {
