...
contract Bridge is Ownable {
    ...
    function depositTokens (uint256 amount) external returns (bool) {
        require(_tokenContract.transferFrom(msg.sender, address(this), amount + _fee),
    ...
    function addCredit (address userAddress, uint256 amount, uint256 depositId) external onlyManager returns (bool) {
        ...
        require(_tokenContract.transfer(userAddress, amount), 'ETNA payment failed');
        return true;
    }

    function withdrawAdmin (uint256 amount) external onlyOwner returns (bool) {
        require(_tokenContract.transfer(msg.sender, amount),
    ...
    function setTokenContract (address tokenAddress) external onlyOwner returns (bool) {
        require(tokenAddress != address(0), 'Token address can not be zero');
        _tokenContract = IERC20(tokenAddress);
        return true;
    }
