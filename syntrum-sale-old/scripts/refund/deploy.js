...
async function main() {
  ...
  const refundContract = await Refund.deploy(
    DEPLOYER
  );
  await refundContract.deployed();
  console.log(`Refund contract address: ${refundContract.address}`);

  await refundContract.setSaleData(deployedContracts.SALE);
}
