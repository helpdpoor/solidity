...
describe("Test 1", function () {
  it("Basic functions testing", async function () {
    ...
    await p2e.connect(signers[10]).playGame(signers[1].address, 1, 55, 66, 1);
    await p2e.connect(signers[10]).playGame(signers[1].address, 1, 55, 66, 1);
    await p2e.connect(signers[10]).playGame(signers[1].address, 1, 55, 66, 1);
    await p2e.connect(signers[10]).playGame(signers[1].address, 1, 55, 66, 1);
    await p2e.connect(signers[10]).playGame(signers[1].address, 1, 55, 66, 1);
    ...
    result = await p2e.getUsage(2);
    expect(Number(result[0])).to.equal(21);
