...
contract Storage is Utils {
    ...
    modifier onlyLiquidationContract() {
        require(msg.sender == _liquidationContractAddress, '63');
        _;
    }
    modifier onlyCollateralOperator() {
        require(
            _managers[msg.sender]
            || msg.sender == _liquidationContractAddress, '631'
        );
        _;
    }
    ...
    address _owner;
    address _nftDepositContractAddress;
    address _liquidationContractAddress;
    ...
    ) external onlyCollateralOperator returns (bool) {
    ...
    function setNftDepositContractAddress (
        address nftDepositContractAddress
    ) external onlyManager returns (bool) {
        require(nftDepositContractAddress != address(0), '75');
        _nftDepositContractAddress = nftDepositContractAddress;
        return true;
    }

    function setLiquidationContractAddress (
        address liquidationContractAddress
    ) external onlyManager returns (bool) {
        require(liquidationContractAddress != address(0), '791');
        _liquidationContractAddress = liquidationContractAddress;
        return true;
    }

    function setLiquidationTime (
        address userAddress, uint256 newTime, bool shouldBeZero, bool shouldNotBeZero
    ) external onlyLiquidationContract returns (bool) {
        if (shouldBeZero) {
            require(_liquidationTime[userAddress] == 0, '54');
        }
        if (shouldNotBeZero) {
            require(_liquidationTime[userAddress] > 0, '56');
        }
        _liquidationTime[userAddress] = newTime;
        return true;
    }
    ...
    function getNftDepositContractAddress () external view returns (address) {
        return _nftDepositContractAddress;
    }

    function getLiquidationContractAddress () external view returns (address) {
        return _liquidationContractAddress;
    }
