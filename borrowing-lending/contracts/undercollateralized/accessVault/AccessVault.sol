...
/**
 * @dev Partial interface of the BAA contract.
 */
interface IBAA {
...
contract AccessVault is Initializable, Storage {
    ...
    function initialize (
        ...
        require(
            exchangeRouterAddress != address(0),
                '2.3'
        );
        _owner = msg.sender;
    ...
    function setMarginSwap (
        ...
        if ( // try to swap immediately with canFail option
            _functionCall(
                baaAddress,
                abi.encodeWithSignature(
                    'accessVaultSwap(uint256,uint256,uint256,uint8,bool)',
                    amount,
                    amountBack,
                    _swapTimeLimit,
                    2,
    ...
    function setSwapTimeLimit (
        ...
        require(swapTimeLimit > 0, 'Swap time limit should not be zero');
    ...
    function _getSwapAmount (
        ...
        uint256 amount = IExchangeRouter(_exchangeRouterAddress).getSwapAmount(
            address(0), tokenInAddress, tokenOutAddress, amountIn
        );
        return amount;
    }
    ...
    function isBaa (address baaAddress) public view returns (bool) {
        return _baaRegistry[baaAddress].ownerAddress != address(0);
    }
}