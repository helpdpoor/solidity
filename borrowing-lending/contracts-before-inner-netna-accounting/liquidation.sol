...
contract Liquidation is Collateral {
    function addFlagForLiquidation (
        ...
        require(_userLiquidation(userAddress, false), 'Liquidation requirements is not met');
        require(_liquidationTime[msg.sender] == 0, 'User is already flagged for liquidation');
    ...
    function removeFlagForLiquidation (
        ...
        require(!_userLiquidation(userAddress, true), 'User is at liquidation');
        require(_liquidationTime[msg.sender] > 0, 'User is not flagged for liquidation');
    ...
    function liquidate (
        ...
        require(_liquidationTime[userAddress] > 0,
            'User was not flagged for a liquidation');
        require(_liquidationTime[userAddress] < block.timestamp,
            'Liquidation period is not over yet');
        ...
        require(
            ...
            'Liquidation requirements is not met'
        );
        ...
        if (liquidationUsdAmount > 0 && collateralIndexes[3] > 0) {
            liquidationUsdAmount = _proceedCollateralLiquidation(
                userAddress, msg.sender, collateralIndexes[3], liquidationUsdAmount
            );
        }
        delete _liquidationTime[userAddress];
