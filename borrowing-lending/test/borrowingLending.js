...
describe("borrowingLending.js - Testing debank contracts", function () {
  beforeEach(async function () {
    ...
    await blContract.connect(d.owner).addToManagers(signers[9].address);
    await collateralContract.connect(d.owner).addToManagers(signers[9].address);
    await expect(
  ...
  it("Liquidation", async function () {
    ...
    await collateralContract.connect(d.owner).addToLiquidators(signers[8].address);
    result = await collateralContract.isLiquidator(signers[8].address);
  ...
  it("Admin settings", async function () {
    ...
    await blContract.connect(d.owner).addToManagers(signers[3].address);
    result = await blContract.isManager(signers[3].address);
    expect(result).to.be.true;
    await expect(
    ...
    await collateralContract.connect(d.owner).addToManagers(signers[3].address);
    result = await collateralContract.isManager(signers[3].address);
    expect(result).to.be.true;
    await expect(
    ...
    await blContract.connect(d.owner).removeFromManagers(signers[3].address);
    result = await blContract.isManager(signers[3].address);
    expect(result).to.be.false;
    await collateralContract.connect(d.owner).removeFromManagers(signers[3].address);
    result = await collateralContract.isManager(signers[3].address);
    expect(result).to.be.false;

    result = await collateralContract.isLiquidator(signers[3].address);
    expect(result).to.be.false;
    await collateralContract.connect(d.owner).addToLiquidators(signers[3].address);
    result = await collateralContract.isLiquidator(signers[3].address);
    expect(result).to.be.true;
    await expect(
    ...
    await collateralContract.connect(d.owner).removeFromLiquidators(signers[3].address);
    result = await collateralContract.isLiquidator(signers[3].address);
    expect(result).to.be.false;
    ...
    await collateralContract.connect(d.owner).setLiquidationManager(signers[9].address);
    result = await collateralContract.getLiquidationManager();
    expect(result).to.equal(signers[9].address);
    await expect(
      collateralContract.connect(d.owner).setLiquidationManager(zeroAddress)
    )
    ...
    expect(result).to.equal(signers[0].address);
    await blContract.connect(signers[0])
      .transferOwnership(d.owner.address);
    result = await blContract.owner();
    expect(result).to.equal(d.owner.address);
  ...
  it("Manager settings", async function () {
    ...
    expect(result).to.equal(d.owner.address);
    await nftCollateralContract.connect(signers[9])
      .setMarketplaceContract(d.owner.address);
    result = await nftCollateralContract.getMarketplaceContract();
    expect(result).to.equal(d.owner.address);
    await nftCollateralContract.connect(signers[9])
      .setNftContract(d.owner.address);
    result = await nftCollateralContract.getNftContract();
    expect(result).to.equal(d.owner.address);
  });
  ...
  it("Admin withdraw/replenish", async function () {
    ...
    result = await nftCollateralContract.connect(d.owner)
    ...
    const borrowing1BalanceS10 = Number(ethers.utils.formatUnits(
      await borrowing1Contract.balanceOf(d.owner.address), 6
    ));
  ...
  it("NftCollaterals migration", async function () {
    await nftCollateralContract.connect(d.owner).addToManagers(signers[9].address);
  ...
  it("Lending - borrowing using tokens with different decimals", async function () {
    ...
    await blContract.connect(d.owner).setUsdRateData(
      borrowing3Contract.address,
      ethers.utils.parseUnits('1', 18 + (18 - borrowing3Decimals)),
      false
    );

    await collateralContract.connect(d.owner).addCollateralProfile(
    ...
    await d.ratesContract.connect(d.owner)
      .setUsdRate(
        erc20Collateral2Contract.address,
        ethers.utils.parseUnits('0.5', 28)
      );
