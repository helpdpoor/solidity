...
contract CollateralWithLiquidationContract is UtilsContract {
    ...
    // part of the liquidation fee that will be paid to liquidators (the rest to admin) (% * 100)z
    ...
    uint8 internal _collateralTypesNumber;
    ...
    function addCollateralProfile (
        ...
        if (contractAddress == address(0)) collateralType = 0;
            ...
            require (
                _collateralTypesNumber < 2 ** 8,
                    'Creating new collateral type is not possible'
            );
            _collateralTypesNumber ++;
            collateralType = _collateralTypesNumber - 1;
    ...
    function setNoFee (uint8 collateralType, bool noFee) external onlyManager returns (bool) {
        _noFee[collateralType] = noFee;
        return true;
    }
    ...
    function getCollateralTypesNumber () external view returns (uint8) {
        return _collateralTypesNumber;
    }
    ...
    function getUserCollateralUsdAmount (
        ...
        for (uint256 i = 1; i <= _collateralProfilesNumber; i ++) {
            ...
            if (_usersCollateralIndexes[userAddress][i] == 0) continue;
            uint256 amount = getCollateralUsdAmount(
    ...
    function getTotalCollateralUsdAmounts (
        ...
        for (uint256 i = 1; i <= _collateralProfilesNumber; i ++) {
            uint256 collateralIndex = _usersCollateralIndexes[userAddress][i];
            if (collateralIndex == 0 || _collaterals[collateralIndex].liquidated
                || _collaterals[collateralIndex].amount == 0) continue;
        ...
        return (totalCollateralUsdAmount, feeCollateralUsdAmount);
    }
    ...
    function getCollateralLiquidationUsdAmount (
        ...
        for (uint256 i = 1; i <= _collateralProfilesNumber; i ++) {
            ...
            uint256 factor = DECIMALS + _collateralProfiles[i].liquidationFactor;
        ...
        return collateralLiquidationUsdAmount;
    }
    ...
    function liquidate (
        ...
        for (uint256 i = 1; i <= _collateralProfilesNumber; i ++) {
            ...
            collateralLiquidationUsdAmount += _collaterals[collateralIndex].amount
                ...
                * DECIMALS
                / SHIFT
                / (DECIMALS + _collateralProfiles[i].liquidationFactor);
