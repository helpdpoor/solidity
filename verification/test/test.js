...
let signers, tokensArray, proxyContract, etnaContract, nEtnaContract, erc20CollateralContract, lpContract, marketplaceContract, nftContract, borrowing1Contract, borrowing2Contract, collateralContract, nftCollateralContract, blContract, rewardContract, result;
...
describe("Testing contract", function () {
  ...
  //     .borrow(1, ethers.utils.parseUnits(borrowing1Amount.toString()), false);
  //
  //   result = await collateralContract.userLiquidation(signers[1].address, false);
  ...
  //     / ((1 + collateral1LiquidationFactor));
  ...
  //
  //   await expect(
  //     collateralContract.connect(signers[10]).addFlagForLiquidation(signers[1].address)
  //   ).to.be.revertedWith('77');
  //
  //   await expect(
  //     collateralContract.connect(signers[7]).addFlagForLiquidation(signers[1].address)
  ...
  //
  //   await expect(
  //     collateralContract.connect(signers[7]).addFlagForLiquidation(signers[0].address)
  //   ).to.be.revertedWith('53');
  //
  //   await collateralContract.connect(signers[7]).addFlagForLiquidation(signers[1].address);
  //
  ...
  //     await collateralContract.getLiquidationFee(), 4
  //   ));
  //   const liquidatorPercentage = Number(ethers.utils.formatUnits(
  //     await collateralContract.getLiquidatorPercentage(), 4
  //   ));
  //
  //   await hre.timeAndMine.increaseTime(`1 days`);
  //   await signers[0].sendTransaction({
  ...
  //     await collateralContract.getLiquidationFlagMargin(), 4
  //   ));
  ...
  //   await blContract.connect(signers[9]).setUsdRateData(
  //     borrowing1Contract.address,
  //     ethers.utils.parseUnits((marginBorrowingUsdRate * 0.9).toString()),
  //     false
  //   );
  //   result = await collateralContract.userLiquidation(signers[1].address, false);
  //   expect(result).to.be.false;
  //   await blContract.connect(signers[9]).setUsdRateData(
  //     borrowing1Contract.address,
  //     ethers.utils.parseUnits(marginBorrowingUsdRate.toString()),
  //     false
  //   );
  //   result = await collateralContract.userLiquidation(signers[1].address, false);
  //   expect(result).to.be.true;
  //   result = await collateralContract.getUserLiquidationTime(signers[1].address);
  //   expect(Number(result)).to.equal(0);
  //
  //   await collateralContract.connect(signers[7]).addFlagForLiquidation(signers[1].address);
  //   result = await collateralContract.getUserLiquidationTime(signers[1].address);
  ...
  //     1, ethers.utils.parseUnits((toBeReturnedBorrowingAmount * 0.99).toString().substring(0, 20)), false
  //   );
  //   result = await collateralContract.getUserLiquidationTime(signers[1].address);
  //   expect(Number(result)).to.be.greaterThan(0);
  //   await blContract.connect(signers[1]).returnBorrowing(
  //     1, ethers.utils.parseUnits((toBeReturnedBorrowingAmount * 0.011).toString().substring(0, 20)), false
  //   );
  //
  //   result = await collateralContract.getUserLiquidationTime(signers[1].address);
  //   expect(Number(result)).to.equal(0);
  ...
  //   result = await collateralContract.userLiquidation(signers[2].address, false);
  //   expect(result).to.be.false;
  //
  //   await blContract.connect(signers[9]).setUsdRateData(
  //     borrowing1Contract.address,
  ...
  //   result = await collateralContract.userLiquidation(signers[2].address, false);
  //   expect(result).to.be.true;
  //   await collateralContract.connect(signers[7]).addFlagForLiquidation(signers[2].address);
  //
  //   await hre.timeAndMine.increaseTime(`1 days`);
  //   await signers[0].sendTransaction({
  //     to: signers[1].address,
  ...
  //     );
  //
  //   result = await collateralContract.userLiquidation(signers[0].address, false);
  //   expect(result).to.be.true;
  //   result = await collateralContract.userLiquidation(signers[1].address, false);
  //   expect(result).to.be.true;
  //   result = await collateralContract.userLiquidation(signers[2].address, false);
  ...
  //   await collateralContract.connect(signers[10])
  //     .addToLiquidators(signers[8].address);
  //   await collateralContract.connect(signers[7])
  ...
  //     444, 555, 666, 777
  //   );
  //   result = await collateralContract.getLiquidationFee();
  //   expect(Number(result)).to.equal(444);
  //   result = await collateralContract.getLiquidatorPercentage();
  //   expect(Number(result)).to.equal(555);
  //   result = await collateralContract.getLiquidationFlagMargin();
  //   expect(Number(result)).to.equal(666);
  //   result = await collateralContract.getLiquidationPeriod();
  ...
  //   let totalReward = 0;
  //   await blContract.connect(signers[0])
  //     .lend(1, ethers.utils.parseUnits(lendingAmounts[0][1].toString()));
  //   await blContract.connect(signers[1])
  ...
  //     .lend(1, ethers.utils.parseUnits(lendingAmounts[0][1].toString()));
  //   await blContract.connect(signers[1])
  //     .lend(1, ethers.utils.parseUnits(lendingAmounts[1][1].toString()));
  //   await blContract.connect(signers[2])
  //     .lend(2, ethers.utils.parseUnits(lendingAmounts[2][2].toString()));
  //   await blContract.connect(signers[3])
  //     .lend(1, ethers.utils.parseUnits(lendingAmounts[3][1].toString()));
  //   await blContract.connect(signers[3])
  //     .lend(2, ethers.utils.parseUnits(lendingAmounts[3][2].toString()));
  //   await blContract.connect(signers[4])
  //     .lend(1, ethers.utils.parseUnits(lendingAmounts[4][1].toString()));
  //   await blContract.connect(signers[4])
  ...
  //   await blContract.connect(signers[3])
  //     .withdrawLending(1, ethers.utils.parseUnits((lendingAmounts[3][1] / 2).toString()));
  //
  //   expect(roundToPrecision(Number(ethers.utils.formatUnits(
  //     await rewardContract.calculateReward(signers[0].address, true)
  //   )), 5)).to.equal(roundToPrecision(rewards[0], 5));
  ...
  //     .setReserves(
  //       ethers.utils.parseUnits('100000'.toString()),
  //       ethers.utils.parseUnits('200000'.toString())
  //     );
  //
  //   await proxyContract.connect(signers[10])
  //     .setUsdRateData(
  ...
  it("Proxy advanced testing", async function () {
    const LP = await ethers.getContractFactory("LPToken");
    const lpEtnaBnb = await LP.deploy(signers[10].address, 'LP token', 'LP token', ethers.utils.parseUnits('54659.0534219587'), 18);
    await lpEtnaBnb.deployed();
    await lpEtnaBnb.connect(signers[10])
      .setReserves(
        ethers.utils.parseUnits('5942896.0354738266'),
        ethers.utils.parseUnits('951.5992502982')
      );
    const lpEtnaBusd = await LP.deploy(signers[10].address, 'LP token', 'LP token', ethers.utils.parseUnits('160425.9985512401'), 18);
    await lpEtnaBusd.deployed();
    await lpEtnaBusd.connect(signers[10])
      .setReserves(
        ethers.utils.parseUnits('683075.1217108559'),
        ethers.utils.parseUnits('45038.8065613038')
      );
    ...
    let result = await proxyContract.getUsdRate(lpEtnaBusd.address);
    console.log(Number(ethers.utils.formatUnits(
      result
    )));
  });
});
