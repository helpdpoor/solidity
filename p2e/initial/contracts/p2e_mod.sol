...
contract p2e is Ownable {
    ...
    function setGameResult (uint256 gameId, bool win) external onlyOwner returns (bool) {
        _games[gameId].win = win;
        uint256 _winAmount = _winAmounts[_games[gameId].level - 1];
        if (win) {
            _userWin[_games[gameId].playerAddress] += _winAmount;
