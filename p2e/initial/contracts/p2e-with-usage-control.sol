...
contract p2e is Ownable {
    ...
    mapping (address => uint256) _userLastGameId; // User's last game id
    ...
    mapping (uint256 => Usage) _tokenUsage; // token usage by address for character tokens
    ...
    mapping (address => uint256) _userGameCounter;
    mapping (uint256 => Game) _userGames;
    IERC20 ETNA;
    ...
    function setUsage (address userAddress, uint256 characterId) internal returns (bool) {
        if (_tokenUsage[characterId].lastAddress != userAddress) {
            require(block.timestamp - _tokenUsage[characterId].lastTime > 86400,
                'This asset was used by another address less than 24 hours ago');
        }
        if (block.timestamp - _tokenUsage[characterId].lastTime <= 86400) {
            _tokenUsage[characterId].number += 1;
        } else {
            _tokenUsage[characterId].number = 1;
        }
        if (_tokenUsage[characterId].number > _gamePerPeriodNumber) {
            if (_paidGameNumber[userAddress] > 0) {
                _paidGameNumber[userAddress] -= 1;
            } else {
                require(_gamePayment[userAddress] > 0,
                    'This character can not be used. Wait 24 hours for resetting or pay for extra usage');
                _gamePayment[userAddress] = 0;
                _paidGameNumber[userAddress] = _gamePerPeriodNumber - 1;
            }
        }
        _tokenUsage[characterId].lastTime = block.timestamp;
        _tokenUsage[characterId].lastAddress = userAddress;
        return true;
    }

    function getUsage (uint256 characterId) external view returns (uint256, uint256, address) {
        Usage memory usage = _tokenUsage[characterId];
        return (usage.number, usage.lastTime, usage.lastAddress);
    }
    ...
    function playGame (
        ...
        require(setUsage(userAddress, characterId));
        _withdrawTaxResetTime[userAddress] = block.timestamp;
        _gameCounter ++;
        _userGameCounter[userAddress] ++;
        Game memory newGame = Game({
            id: _gameCounter,
            price: _gamePayment[userAddress],
        ...
        _userGames[_userGameCounter[userAddress]] = newGame;
        _userLastGameId[newGame.playerAddress] = _gameCounter;
    ...
    function getUserLastGameId (address userAddress) external view returns (uint256) {
        return _userLastGameId[userAddress];
    }
    ...
    function getGame (uint256 gameId) external view returns (address, uint256, uint256, uint256, bool) {
        ...
        return (game.playerAddress, game.price, game.characterId, game.level, game.win);
    }

    function getLastUserGameId (address userAddress) external view returns (uint256) {
        return _userGameCounter[userAddress];
    }

    function getUserGame (uint256 userGameId) external view returns (address, uint256, uint256, uint256, bool) {
        Game memory game = _userGames[userGameId];
        return (game.playerAddress, game.price, game.characterId, game.level, game.win);
    }
