...
describe('Vesting contract testing', function () {
  beforeEach(async function () {
    ...
    d.paymentToken = await d.Token.deploy(
      d.owner.address,
      ethers.utils.parseUnits('1000000'),
      'Fee token',
      'Fee token'
  ...
  it('Vesting, payment in tokens', async function () {
    d.decimals = 18;
    d.blockNumber = await ethers.provider.getBlockNumber();
    d.block = await ethers.provider.getBlock(d.blockNumber);
    d.now = d.block.timestamp;
    ...
    await d.paymentToken.connect(d.allocationCreator).approve(
      d.vesting.address,
      ethers.utils.parseUnits('100')
    );
    await d.vestingToken.connect(d.allocationCreator).approve(
    ...
    expect(Number(ethers.utils.formatUnits(
      await d.paymentToken.balanceOf(d.feeReceiver.address)
    ))).to.equal(d.balance + (d.feeAmount * (10000 - d.feeDiscount) / 10000));
